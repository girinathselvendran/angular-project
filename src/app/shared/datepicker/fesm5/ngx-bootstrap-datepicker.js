...
var BsDatepickerContainerComponent = /** @class */ (function (_super) {
    ...
    function (day) {
        ...
        /* iinterchange Custom changes start */
        var currentDateString = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
        var currentDateTime = new Date(currentDateString);
        day.date.setHours(currentDateTime.getHours());
        day.date.setMinutes(currentDateTime.getMinutes());
        day.date.setSeconds(currentDateTime.getSeconds());
        /* iinterchange Custom changes  end */
        this._store.dispatch(this._actions.select(day.date));
    };
    /**
     * @return {?}
...
var BsDatepickerInputDirective = /** @class */ (function () {
    ...
    function (value) {
        if (!value) {
            ...
            /* iinterchange Custom changes start */
            var customDateInputFormat = this._picker._config.dateInputFormat;
            if(!(value instanceof Date && !isNaN(value.valueOf())))
            {
                var splitDateStrings = value.split("-");
                if(splitDateStrings.length >= 2)
                {
                    if(splitDateStrings[1] !== null && splitDateStrings[1] !== undefined)
                    {
                        if(!isNaN(splitDateStrings[1]))
                        {
                            customDateInputFormat = customDateInputFormat.replace("MMM","MM");
                        }
                    }
                }
                else{
                    splitDateStrings = value.split("/");
                    if(splitDateStrings.length >= 2)
                    {
                        if(splitDateStrings[1] !== null && splitDateStrings[1] !== undefined)
                        {
            ...
            /* iinterchange Custom changes end */
            this._value = parseDate(value, customDateInputFormat, this._localeService.currentLocale);
            if (customDateInputFormat.includes("HH:mm"))
            {
                console.log("Date value: "+this._value);
            }
        }
